...
namespace epic7
{
    ...
    /// 大漠插件C#免註冊調用類
    /// 作者：清風撫斷雲
    /// 本模塊必須包含dmc.dll 實現不用註冊dm.dll 到系統可以動態調用
    /// 最新修改：2013-8-23
    /// 修復vs2012 調試自動退出問題
    /// </summary>
    class CDmSoft : IDisposable
    {
        ...
        ///在指定的Foobar窗口繪製gif動畫.
        ///</summary> 
        ///<param name="hwnd">指定的Foobar窗口,注意,此句柄必須是通過CreateFoobarxxxx系列函數創建出來的</param> 
        ///<param name="x">左上角X坐標(相對於hwnd客戶區坐標)</param> 
        ///<param name="y">左上角Y坐標(相對於hwnd客戶區坐標)</param> 
        ///<param name="pic_name">圖像文件名</param> 
        ///<param name="repeat_limit">表示重複GIF動畫的次數，如果是0表示一直循環顯示.大於0，則表示循環指定的次數以後就停止顯示.</param> 
        ///<param name="delay">表示每幀GIF動畫之間的時間間隔.如果是0，表示使用GIF內置的時間，如果大於0，表示使用自定義的時間間隔.</param> 
        ...
        ///<summary> 
        ///綁定指定的窗口,並指定這個窗口的屏幕顏色獲取方式,鼠標仿真模式,鍵盤仿真模式 高級用戶使用.
        ///</summary> 
        ///<param name="hwnd">指定的窗口句柄</param> 
        ///<param name="display">屏幕顏色獲取方式</param> 
        ///<param name="mouse">鼠標仿真模式</param> 
        ///<param name="keypad">鍵盤仿真模式</param> 
        ///<param name="public_desc">公共屬性 dx模式共有</param> 
